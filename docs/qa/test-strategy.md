...
- Type/static: TypeScript `checkJs`、ESLint、構文、manifest、SW v13 precache、CSP、core/expansion data validation、Evidence Sufficiencyを検証する。
...
Chrome/Edge/managed WebKitの成功からSafari/Firefoxや実端末の成功を推定しません。axeはVoiceOver/NVDA/TalkBackによる実読み上げの代替ではありません。個人利用分類では主環境を必須とし、範囲外targetは`OUT_OF_SCOPE`または`NOT_RUN`、host起動不能は`ENVIRONMENT_BLOCKED`として保持します。
